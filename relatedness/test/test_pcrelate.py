...
class Platform(unittest.TestCase):
    ...
    execution = "platform"
    profile_name = 'bdc'
    log_filename = 'UnitTest_pcrelate'
    ...
    def setUpClass(cls):
        ...
        cls.APP = 'smgogarten/genesis-relatedness/pc-relate'
        cls.inputs = {}
        cls.TASK_NAME = 'UnitTest_pcrelate'
        ...
        cls.inputs['pca_file'] = cls.session.files.query(
            names=['pcair.RData'],
            project=cls.project
        )[0]
        ...
        cls.inputs['n_pcs'] = 3
        cls.inputs['variant_block_size'] = 1024
        cls.inputs['n_sample_blocks'] = 2
    ...
    def test_outputs(self):
        ...
        if self.task.status == 'COMPLETED':
            ...
            out_expected_name = 'unittest_pcrelate.RData'
            out_name = self.task.outputs['pcrelate_output'].name
            ...
            out_expected_name = 'unittest_pcrelate_Matrix.RData'
            out_name = self.task.outputs['pcrelate_matrix'].name
            if out_name.startswith('_'):
                self.assertEqual(out_expected_name, '_'.join(out_name.split('_')[2:]))
            else:
                self.assertEqual(out_expected_name, out_name)
            out_expected_name = 'unittest_pcrelate_all.pdf'
            out_name = self.task.outputs['pcrelate_plots'][0].name
            if out_name.startswith('_'):
                self.assertEqual(out_expected_name, '_'.join(out_name.split('_')[2:]))
            else:
                self.assertEqual(out_expected_name, out_name)
            self.assertTrue(len(self.task.outputs['pcrelate_plots']) > 1)
