...
class Platform(unittest.TestCase):
    ...
    execution = "platform"
    profile_name = 'bdc'
    log_filename = 'UnitTest_KING_robust'
    ...
    def setUpClass(cls):
        ...
        cls.APP = 'smgogarten/genesis-relatedness/king-robust-1'
        cls.inputs = {}
        cls.TASK_NAME = 'UnitTest_KING_robust'
    ...
    def test_outputs(self):
        ...
        if self.task.status == 'COMPLETED':
            ...
            out_expected_name = 'unittest_king_robust.gds'
            out_name = self.task.outputs['king_robust_output'].name
            ...
            out_expected_name = 'unittest_king_robust_all.pdf'
            out_name = self.task.outputs['king_robust_plots'][0].name
            if out_name.startswith('_'):
                self.assertEqual(out_expected_name, '_'.join(out_name.split('_')[2:]))
            else:
                self.assertEqual(out_expected_name, out_name)
            self.assertTrue(len(self.task.outputs['king_robust_plots']) > 1)
